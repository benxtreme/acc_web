<?php 
require 'header.php';
?>

<main id="main-content">
    <div class="dialog-off-canvas-main-canvas" data-off-canvas-main-canvas>
        <div id="block-anagkazo-college-content">



            <div class="basic-page-node color-theme--crimson">





                <div class="page-header-plain c-panel c-panel--parchment ">
                    <div class="o-container-fluid c-panel--line">
                        <div class="o-row page-header-plain__content">

                            <div
                                class="page-header-plain__title  o-col-sm-11 o-col-sm--offset-1 page-header-plain__title--breadcrumbs page-header__breadcrumbs">
                                <h1 class="title-serif--medium-xl"><span property="schema:name">Contact ABMTC</span>
                                </h1>

                                <div class="c-breadcrumb">

                                </div>
                            </div>



                        </div>
                    </div>
                </div>



                <div class="page-body">


                    <section
                        class="paragraph paragraph--type--webform-reference paragraph--view-mode--default c-panel--box"
                        id="thank-you-for-your-interest-in-anagkazo-college">
                        <span class="c-panel--box-outline"></span>

                        <div class="o-container-fluid c-panel--line">
                            <div class="c-panel--box-contained--content">
                                <div class="m-content-section__lead">
                                    <h2 class="title-serif--large">

                                        Thank you for your interest in ABMTC!
                                    </h2>
                                </div>
                                <div class="c-wysiwyg-section">


                                    <p>We receive a very large number of enquiries everyday and may not always be able
                                        to respond as quickly as we would like. Please kindly send a message through
                                        Email or WhatsApp.</p>

                                </div>
                            </div>
                        </div>
                    </section>
                    <br>

                    <section id="contact-information" class="m-content-section c-panel--red">
                        <span class="c-panel--box-outline"></span>
                        <div class="o-container-fluid c-panel--line c-panel--line--lg m-content-section__content">

                            <div class="c-panel--box-contained--content">
                                <div class="m-content-section__lead">
                                    <h2 class="title-serif--large">

                                        Contact Information
                                    </h2>
                                </div>


                                <div class="c-wysiwyg-section ">


                                    <h3>Contact ABMTC by WhatsApp:</h3>
                                    <p>+233557467460</p>
                                    <p>+233594900295</p>

                                    <h3>Contact ABMTC by Email:</h3>
                                    <p>abmtcrecruitment@gmail.com</p>
                                </div>

                            </div>
                        </div>
                    </section>
                    <section id="virtual-tour"
            class="o-two-block  o-two-block--hidden-title  c-panel--pad--large m-two-block-content">


            <div class="o-container-fluid ">

              <h2 class="u-sr-only">

                Virtual Tour
              </h2>

              <div class="o-two-block__blocks">
                

                <div class="o-two-block__block o-two-block__block--right ">

                  <div class="c-teaser-medium">
                    <a href="#" class="c-teaser-medium__link">
                      <div class="c-teaser-medium__header c-teaser-header u-watch">
                        <h3 class="title-serif--medium c-teaser-medium__title">Take a virtual tour</h3>

                        <span class="c-teaser-medium__arrow" aria-hidden="true"><svg xmlns="http://www.w3.org/2000/svg"
                            width="16" height="11" viewBox="0 0 16 11">
                            <title> </title>
                            <polygon
                              points="13.293 4.735 9.265 .707 9.972 0 15.21 5.238 9.966 10.395 9.265 9.682 13.278 5.735 0 5.735 0 4.735" />
                          </svg>
                        </span>
                      </div>
                      <div class="c-teaser-medium__image">

                        <img src="assets/img/gate.jpg" width="800" height="588" alt="Main gate" typeof="foaf:Image" />
                        <noscript><img src="assets/img/gate.jpg" alt="Main gate" /></noscript>
                      </div>
                    </a>
                  </div>

                </div>
              </div>


            </div>
          </section>

                </div>


            </div>

        </div>



    </div>

</main>

<?php
require 'footer.php';
?>